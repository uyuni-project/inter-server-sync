...
func getJoinsClause(path []string, schemaMetadata map[string]schemareader.Table) string {
	...
	for i := 0; i < len(reversePath)-1; i++ {
		...
		relationFound := findRelationInfo(schemaMetadata[firstTable].ReferencedBy, secondTable, firstTable)
		if relationFound == nil {
			relationFound = findRelationInfo(schemaMetadata[firstTable].References, secondTable, firstTable)
...
func findRelationInfo(References []schemareader.Reference, tableToFind string, sourceTable string) map[string]string {
